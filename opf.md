...
Let $$y_{ij}^{net} = y^M_{ij}+(y_{ij}+y^s_{ij}) \cdot \left\lvert \tau^i_{ij}\right\lvert^2$$, $$y_{ji}^{net} = y_{ij}+y^s_{ij}$$, and $$2\mathcal{K}=\{i,j\} \in \mathcal{K} \bigcup \{j,i\} \in \mathcal{K}$$. The admittance matrix can be written as shown below:
